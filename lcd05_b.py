...
def display0Screen():
    ...
    for x in range(10):
        if On[x]==1:
            ...
            lcd.message(str(x+1))
    print "0 screen"
    print screen def display1Screen():
    lcd.setCursor(3,0)
    ...
    if arrow == 1:
        lcd.setCursor(0,1)
        lcd.message("-->")
    print "1 screen" def display2Screen():
    lcd.setCursor(5,0)
    lcd.message("Alarm ")
    ...
    if Darrow == 0:
        ...
        lcd.message(">")
        lcd.setCursor(17,2)
        lcd.message("<") def display3Screen():
    global screen
    lcd.setCursor(4,1)
    lcd.message("Alarm Set")
    time.sleep(1)
    lcd.clear()
    screen=0 def display4Screen():
    global screen
    lcd.setCursor(4,1)
    lcd.message("Alarm Off")
    time.sleep(1)
    lcd.clear()
    screen=0 def display5Screen():
    lcd.setCursor(3,0)
    lcd.message("Alarm!")
    lcd.setCursor(3,2)
    lcd.message(time.strftime("%H:%M:%S", time.localtime(time.time()))) def display6Screen():
    for x in range(0, 4):
        lcd.setCursor(3,x)
        lcd.message("Alarm ")
    ...
    lcd.message("-->") def display7Screen():
    for x in range(0, 4):
        lcd.setCursor(2,x)
        lcd.message("Alarm Off")
        lcd.message(str(x + Scroll+ 1))
        ...
        lcd.message(")")
    lcd.setCursor(0,AArrow)
    lcd.message("->") def display8Screen():
    lcd.setCursor(4,0)
...
def button():
            ...
            if screen ==1:
 		    ...
 		    selected=selected + 1
                ...
                if AlarmMusicList[Aselected]< 2:
                    AlarmMusicList[Aselected]=AlarmMusicList[Aselected]+1
            lcd.clear()
        time_stamp_3 = time.time() ifplayed = 1 def Alarm():
    global On
    global Sound
    global screen
    global LastSound
    ...
    if Sound==1:
        ...
        if time.time()-Nowtime > 60:
         ...
         ifplayed = 0
         pygame.mixer.stop()
    print HRN, MINN, SECN, AHR, AMIN, ASEC, Sound, ifplayed while 1:
    Alarm()
    button()
    if screen == 0:
        display0Screen()
